...
@section('content')
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <div class="inline-flex flex-wrap gap-0 rounded-md overflow-hidden border border-gray-300 shadow-sm">
                    ...
                    <a href="javascript:;" data-target="#laboratory-paper-masul-create-modal" data-toggle="modal"
                        ...
                        title="Laboratoriyaga masul biriktirish">
                ...
                <tbody>
                    ...
                    @foreach ($laboratorys as $xodimlar)
                        ...
                        <tr class="intro-x">
                            ...
                            <td>
                                <a href="" class="font-medium ">{{ $xodimlar->user->name ?? $xodimlar->legacyUser->name ?? "Ma'sul biriktirilmagan" }}</a>
                            </td>
    ...
    <div class="modal" id="laboratory-paper-masul-create-modal">
        <div class="modal__content modal__content--xl">
            ...
            <div class="p-5">
                <form id="laboratory-masul-create-form" method="POST" action="{{ route('laboratory_masul.store') }}" class="validate-form" novalidate="novalidate">
                    @csrf
                    <input type="hidden" name="form_source" value="laboratory_masul_create">
                    @if ($errors->any() && old('form_source') === 'laboratory_masul_create')
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <div class="relative">
                                <input type="password" name="password" id="laboratory-create-password" class="input w-full border pr-10 @error('password') border-red-500 @enderror">
                                <button type="button" class="absolute inset-y-0 right-0 flex items-center px-3 password-toggle" data-target="laboratory-create-password" tabindex="-1">
            ...
            <div class="px-5 py-4 bg-gray-50 border-t border-gray-200 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
                ...
                <button type="submit" form="laboratory-masul-create-form" class="button bg-theme-1 text-white flex items-center gap-2">
        ...
        <div class="modal__content modal__content--xl">
            ...
            <div class="p-5">
                <div class="flex items-center gap-3 mb-5 p-4 bg-gray-50 rounded-lg border">
                    <div id="laboratory-edit-initials" class="w-11 h-11 rounded-full bg-theme-1 text-white flex items-center justify-center font-semibold text-sm"></div>
                    <div class="min-w-0">
                        <p id="laboratory-edit-preview-name" class="font-medium truncate"></p>
                        <p id="laboratory-edit-preview-email" class="text-sm text-gray-600 truncate"></p>
                    </div>
                </div>
                <form id="laboratory-masul-edit-form" method="POST" action="" novalidate="novalidate">
                    @csrf @method('PUT')
                    <input type="hidden" name="email" id="laboratory-edit-email-hidden">
                    <div class="grid grid-cols-12 gap-4 mb-5">
                        <div class="col-span-12 sm:col-span-6">
                            <label class="block text-sm font-medium mb-1">F.I.Sh <span class="text-red-600">*</span></label>
                            <input type="text" name="name" id="laboratory-edit-name" class="input w-full border" required>
                        </div>
                        <div class="col-span-12 sm:col-span-6">
            ...
            <div class="px-5 py-4 bg-gray-50 border-t flex justify-end gap-2">
                <button type="button" data-dismiss="modal" class="button border text-gray-700">Bekor qilish</button>
                <button type="submit" form="laboratory-masul-edit-form" class="button bg-theme-1 text-white flex items-center gap-2">
        ...
        <div class="modal__content modal__content--xl">
            <div class="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600"><span id="laboratory-masul-count-visible">{{ $masullar->count() }}</span> / {{ $masullar->count() }} ta</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" class="button bg-theme-1 text-white text-sm hidden sm:flex items-center gap-1 laboratory-masul-quick-add">
            ...
            <div class="px-5 py-3 border-b bg-gray-50">
                <div class="search relative">
                    <input type="text" id="laboratory-masul-search" class="search__input input placeholder-theme-13 w-full" placeholder="Ism yoki email bo'yicha qidirish...">
            ...
            <div class="p-5 max-h-[60vh] overflow-y-auto">
                ...
                @empty
                    <div class="text-center py-12 text-gray-500">
                        ...
                        <button type="button" class="button bg-theme-1 text-white mt-4 laboratory-masul-quick-add">Biriktirish</button>
                    </div>
                @endforelse
                <div id="laboratory-masul-no-results" class="hidden py-12 text-center text-gray-500">Natija topilmadi</div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editForm = document.getElementById('laboratory-masul-edit-form');
            const createForm = document.getElementById('laboratory-masul-create-form');
            const editUrlBase = @json(url('/laboratory-masul-edit'));
            const hasCreateErrors = @json($errors->any() && old('form_source') === 'laboratory_masul_create');
            ...
            document.querySelectorAll('.laboratory-masul-edit-btn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    editForm.action = editUrlBase + '/' + this.dataset.userId;
                    document.getElementById('laboratory-edit-name').value = this.dataset.userName;
                    document.getElementById('laboratory-edit-email-display').value = this.dataset.userEmail;
                    document.getElementById('laboratory-edit-email-hidden').value = this.dataset.userEmail;
                    document.getElementById('laboratory-edit-password').value = '';
            ...
            const countEl = document.getElementById('laboratory-masul-count-visible');
            const noRes = document.getElementById('laboratory-masul-no-results');
            ...
            document.querySelectorAll('.laboratory-masul-quick-add').forEach(btn => {
        ...
        <div class="modal__content modal__content--xl">
            <div class="p-5">
                <div style="display: flex; justify-content: space-between;margin-bottom: 10px; align-items: center;">
                    <div>
                        <h2 class="text-lg font-medium">Laboratoriya qo'shish</h2>
                    </div>
                    <div>
                            ...
                            <div class="grid grid-cols-12 gap-2">
                                <div class="w-full col-span-12">
                                    <label class="flex flex-col sm:flex-row"> <span
                                            class="mt-1 mr-1 sm:mt-0 text-xs text-red-600">*</span>Laboratoriya nomi
                                    </label>
