...
@section('content')
    ...
    <div class="content">
        <div class="flex justify-between align-center mt-6 mb-6" style="align-items: center;">
            ...
            <h2 class="intro-y text-lg font-medium"> Tashkilotlar soni: {{ $akadem_count ?? 404 }} ta Akademik harakatchanlik soni
                {{ $akadem_expert ?? 0 }} ta</h2>
            ...
            <div class="flex justify-between align-center gap-6">
                <div class="relative text-gray-700">
                    <form action="{{ route('search_akadem') }}" method="GET">
                ...
                <form method="GET" action="{{ route('search_akadem') }}">
                    <input type="hidden" name="id" value="{{ $regionId }}">
                    ...
                    <tbody>
                        @forelse ($tashkilotlar as $tashkilots)
                            <tr class="intro-x">
                                ...
                                <td>
                                    <a href="{{ route('akadems.index', ['id' => $tashkilots->id]) }}"
                                ...
                                <td style="text-align: center;">
                                    {{ $tashkilots->akademlar()->where('is_active', 1)->count() }}/{{ $tashkilots->akademexperts()->where('quarter', 1)->count() }}
                                </td>

                                <td style="text-align: center;">
                                    {{ $tashkilots->akademexperts()->where('quarter', 1)->whereIn('status', ['Ijobiy', 'Qoniqarli'])->count() }}/
                                    {{ $tashkilots->akademexperts()->where('quarter', 1)->where('status', 'Salbiy')->count() }}/
                                    {{ $tashkilots->akademexperts()->where('quarter', 1)->where('status', 'Qo‘shimcha o‘rganish talab etiladi')->count() }}
                                </td>

                                <td class="table-report__action w-56">
                                    <div class="flex justify-center items-center">
                                        <a class="flex science-update-action items-center mr-3"
                                            href="{{ route('akadems.index', ['id' => $tashkilots->id]) }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center">Akademik harakatchanlik topilmadi</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
